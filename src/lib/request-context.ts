...
export function getRequestId(req: NextRequest | Request): string {
  ...
  return fromHeader || (typeof crypto !== 'undefined' && 'randomUUID' in crypto ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
}
...
export function getIp(req: NextRequest | Request): string | undefined {
  ...
  return (xfwd.split(',')[0]?.trim() || realIp || undefined) as string | undefined;
}
