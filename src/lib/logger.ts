...
const REDACT_KEYS = new Set([
  ...
  'apiKey', 'apikey', 'accessToken', 'refreshToken', 'session', 'stripeSignature',
]);
...
function redactValue(value: unknown): unknown {
  if (typeof value === 'string') {
    ...
    return value.replace(emailRegex, (_m, user, domain) => `${String(user).slice(0, 1)}***@${domain}`);
  }
  return value;
}
...
export const logger = {
  ...
  error: (message: string, ctx?: LogContext, err?: unknown) => write('error', message, ctx, err),
};
